...
def embed_to_image_lsb(embedded_file: str, cover_img: Image.Image, key: str, encrypt: bool, sequential: bool, metadata_binary: str) -> Tuple[bytes, Image.Image]:
    ...
    if(sequential):
        ...
        location = random_unique_location(
            metadata_length, len(content), seed, width*height*3)
...
def embed_to_image_bpcs(embedded_file: str, cover_img: Image.Image, key: str, encrypt: bool, sequential: bool, threshold: float, metadata_binary: str) -> Tuple[bytes, Image.Image]:
    ...
    width_start = math.ceil(
        math.ceil(math.ceil(metadata_length / 3) / height) / 8)
    ...
    if(sequential):
        ...
        locations = random.sample(
            range(0, len(message_blocks)), len(message_blocks))
        for x in range(width_start, blocks_width):
            if message_pointer >= len(message_blocks):
                break
            for y in range(0, blocks_height):
    ...
    stego_blocks = [[None for _ in range(blocks_width)]
                    for _ in range(blocks_height)]
...
def embed_to_image(embedded_file: str, cover_file: str, key: str, method: str, encrypt: bool, sequential: bool, threshold: float = 0.3) -> Tuple[bytes, Image.Image, float]:
    ...
    metadata_binary = image_metadata_to_binary(
        method, encrypt, sequential, threshold, embedded_file_size, embedded_file_name)
    with Image.open(cover_file) as cover_img:
        original = cover_img.copy()
        cover_capacity_bit, cover_capacity_byte = calculate_image_capacity(
            cover_img, method, threshold)
        if(embedded_file_size + len(metadata_binary) // 8 + 1 > cover_capacity_byte):
            ...
            if(method == 'lsb'):
                stego_img_bytes, stego_img = embed_to_image_lsb(
                    embedded_file, cover_img, key, encrypt, sequential, metadata_binary)
            else:
                stego_img_bytes, stego_img = embed_to_image_bpcs(
                    embedded_file, cover_img, key, encrypt, sequential, threshold, metadata_binary)
...
def extract_from_image_lsb(binary: str, metadata_size: int, encrypt: bool, sequential: bool, embed_file_size: int, key: str, cover_width: int, cover_height: int) -> bytes:
    if sequential:
        ...
        location = random_unique_location(
            metadata_size, embed_file_size*8, seed_generator(key), cover_width*cover_height*3)
        content_binary = ''
        for loc in location:
...
def extract_from_image_bpcs(stego_img: Image.Image, metadata_size: int, encrypt: bool, sequential: bool, embed_file_size: int, key: str, width: int, height: int, conjugation_map: List[int], threshold: float = 0.3) -> bytes:
    width_start = math.ceil(
        math.ceil(math.ceil(metadata_size / 3) / height) / 8)
            ...
            for y in range(0, blocks_height):
                ...
                for i in range(0, 24):
                    if message_blocks_pointer < message_blocks_length:
                        ...
                        break
        message_bin = message_blocks_to_bin(
            message_blocks, conjugation_map, embed_file_size * 8)
    else:
        seed = seed_generator(key)
        random.seed(seed)
        locations = random.sample(
            range(0, message_blocks_length), message_blocks_length)
        for x in range(width_start, blocks_width):
            if message_blocks_pointer >= message_blocks_length:
                break
            for y in range(0, blocks_height):
                ...
                for i in range(0, 24):
                    if message_blocks_pointer < message_blocks_length:
                        if count_bitplane_complexity(blocks[y][x][i]) > threshold:
                            message_blocks[locations[message_blocks_pointer]
                                           ] = blocks[y][x][i]
                            message_blocks_pointer += 1
                        else:
                            continue
...
def extract_from_image(stego_file: str, key: str) -> Tuple[bytes, str]:
    ...
    metadata_size, method, encrypt, sequential, threshold, embed_file_size, embed_file_name = binary_to_image_metadata(
        binary[:metadata_size])
    if(method == 'lsb'):
        content_bytes = extract_from_image_lsb(
            binary, metadata_size, encrypt, sequential, embed_file_size, key, width, height)
    else:
        conjugation_map_length = binary_to_int(
            binary[metadata_size:metadata_size+32])
        conjugation_map = [
            int(i) for i in binary[metadata_size+32:metadata_size+32+conjugation_map_length]]
        content_bytes = extract_from_image_bpcs(stego_img, metadata_size+32+conjugation_map_length,
                                                encrypt, sequential, embed_file_size, key, width, height, conjugation_map, threshold)
