...
def random_unique_location(metadata_length, content_length, seed, cover_size):
    ...
    location = random.sample(
        range(metadata_length, cover_size), content_length)
    return location
...
def cover_to_blocks(cover_img):
    width, height = cover_img.size
    blocks = [[[] for _ in range(math.ceil(width/8))]
...
def block_to_bitplane(block):
    ...
    for x in range(8):
        for y in range(8):
            r, g, b = block[y][x]
            pixel_binary = int_to_binary(
                r) + int_to_binary(g) + int_to_binary(b)
...
def blocks_to_np_img_array(blocks, width, height, depth=3):
    img_array = [[[0 for _ in range(depth)]
